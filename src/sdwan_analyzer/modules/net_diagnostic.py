...
def net_waterfall_info_default(target_url):
    signal.signal(signal.SIGINT, lambda sig,frame: sys.exit(0))

    checker = NetworkStackChecker(target_url, False)
    checker.run_all()

    requests_info = playwright_full_capture(target_url, False, False)
    ...
    print(f"\n📄 报告已保存: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}_network_diagnostic_report.json / .html")

def net_waterfall_info(target_url, headed, verbose):
    signal.signal(signal.SIGINT, lambda sig,frame: sys.exit(0))
...
def main():
    ...
    net_waterfall_info(target_url, headed, verbose)
